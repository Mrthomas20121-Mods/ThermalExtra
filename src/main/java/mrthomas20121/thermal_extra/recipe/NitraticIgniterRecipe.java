...
public class NitraticIgniterRecipe extends ThermalRecipe {

    public NitraticIgniterRecipe(ResourceLocation recipeId, int energy, float experience, List<Ingredient> inputItems, List<FluidIngredient> inputFluids, List<ItemStack> outputItems, List<Float> outputItemChances, List<FluidStack> outputFluids) {

        super(recipeId, energy, experience, inputItems, inputFluids, outputItems, outputItemChances, outputFluids);

        if (this.energy <= 0) {
            int defaultEnergy = NitraticIgniterRecipeManager.instance().getDefaultEnergy();
    ...
    public RecipeSerializer<?> getSerializer() {
        ...
        return ThermalExtraRecipeSerializers.NITRATIC_IGNITER_RECIPE_SERIALIZER.get();
    }
    ...
    public RecipeType<?> getType() {
        ...
        return ThermalExtraRecipeTypes.NITRATIC_IGNITER.get();
    }
