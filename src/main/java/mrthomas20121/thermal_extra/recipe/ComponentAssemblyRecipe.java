...
public class ComponentAssemblyRecipe extends ThermalRecipe {
    ...
    public ComponentAssemblyRecipe(ResourceLocation recipeId, int energy, float experience, List<Ingredient> inputItems, List<FluidIngredient> inputFluids, List<ItemStack> outputItems, List<Float> outputItemChances, List<FluidStack> outputFluids) {

        super(recipeId, energy, experience, inputItems, inputFluids, outputItems, outputItemChances, outputFluids);

        if (this.energy <= 0) {
            int defaultEnergy = ComponentAssemblyRecipeManager.instance().getDefaultEnergy();
    ...
    public RecipeType<?> getType() {
        ...
        return ThermalExtraRecipeTypes.COMPONENT_ASSEMBLY.get();
    }
