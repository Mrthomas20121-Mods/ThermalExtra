...
public class MetalInfuserRecipe extends ThermalRecipe {
    ...
    public MetalInfuserRecipe(ResourceLocation recipeId, int energy, float experience, List<Ingredient> inputItems, List<FluidIngredient> inputFluids, List<ItemStack> outputItems, List<Float> outputItemChances, List<FluidStack> outputFluids) {

        super(recipeId, energy, experience, inputItems, inputFluids, outputItems, outputItemChances, outputFluids);

        if (this.energy <= 0) {
            int defaultEnergy = MetalInfuserRecipeManager.instance().getDefaultEnergy();
    ...
    public RecipeType<?> getType() {
        ...
        return ThermalExtraRecipeTypes.METAL_INFUSER.get();
    }
